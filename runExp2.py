...
def runExp(config):
    ...
    for idx, filename in enumerate (sorted(glob.glob("3Dbrain2/src*.mhd"), key=os.path.getmtime)):
    ...
    for idx, filename in enumerate (sorted(glob.glob("3Dbrain2/tar*.mhd"), key=os.path.getmtime)):
        img = sitk.GetArrayFromImage(sitk.ReadImage(filename))
        img = np.rollaxis(img, 0, 3)
    ...
    from modules.DataSet import DataSet2D, DataSetDeep, DataSetDeepPred
    ...
    deepflashnet = AEModel(net_config = config['net'], 
                        loss_config = config['loss'],
                        device=device)
    deepflashtestnet = AEModel(net_config = config['net'], 
                        loss_config = config['loss'],
                        device=device)
    #deepflashnet.apply(weights_init_uniform)

    for idx, filename in enumerate (sorted(glob.glob("3Dbraintest/src*.mhd"), key=os.path.getmtime)):
        img = sitk.GetArrayFromImage(sitk.ReadImage(filename))
        img = np.rollaxis(img, 0, 3)
        temp = convertTensorformat(img,
                                sourceFormat = 'single3DGrayscale', 
    ...
    for idx, filename in enumerate (sorted(glob.glob("3Dbraintest/tar*.mhd"), key=os.path.getmtime)):
        img = sitk.GetArrayFromImage(sitk.ReadImage(filename))
        img = np.rollaxis(img, 0, 3)
        temp = convertTensorformat(img,
                                sourceFormat = 'single3DGrayscale', 
